...
export default function DashboardClient({
  ...
  const firstName = viewerName.trim().split(/\s+/).at(-1) ?? viewerName;
  const viewerInitials = viewerName
    ...
    .join("");
  const personalizedBalances = balances.map((person, index) =>
    index === 0 ? { ...person, name: viewerName, initials: viewerInitials } : person,
  );
      ...
      <aside className={`sidebar ${mobileOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="sidebar-footer">
          ...
          <div className="profile-row">
            <span className="avatar avatar-photo">{viewerInitials}</span>
            <span><strong>{viewerName}</strong><small>{viewerRole}</small></span>
        ...
        <div className="dashboard-content">
          <section className="welcome-card">
            <div className="welcome-copy">
              ...
              <h2>Chào buổi sáng,<br />{firstName}!</h2>
          ...
          <div className="dashboard-grid">
            ...
            <section className="content-card balance-card">
              ...
              <div className="balance-list">{personalizedBalances.map((person) => <button key={person.name}><span className={`avatar avatar-${person.tone}`}>{person.initials}</span><span className="person-name"><strong>{person.name}</strong><small>{person.amount >= 0 ? "Đã cân đối" : "Cần thanh toán"}</small></span><strong className={person.amount >= 0 ? "money-positive" : "money-negative"}>{formatMoney(person.amount)}</strong></button>)}</div>
